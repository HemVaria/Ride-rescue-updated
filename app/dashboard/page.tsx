...
import { Car, AlertTriangle, MapPin, Clock, Phone, Settings, Plus, Target, Home, Wrench, User, Loader2 } from "lucide-react"
...
export default function DashboardPage() {
  ...
  // Redirect to home if not authenticated, but do it in an effect (not during render)
  useEffect(() => {
    if (!authLoading && !user) {
      router.replace("/")
    }
  ...
  if (!user) {
    // While redirecting, render nothing to avoid setState during render
    return null
  }
    ...
    <div className="min-h-screen text-slate-100">
      <div className="px-6 py-8 pt-32">
        ...
        >
          ...
          <motion.div variants={itemVariants} className="bg-slate-900/80 backdrop-blur-xl rounded-2xl p-6 border border-slate-700/50 shadow-xl">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <motion.div variants={itemVariants}>
              <Card className="bg-slate-900/80 backdrop-blur-xl border-slate-700/50 shadow-xl h-full">
                <CardHeader>
                  ...
                  <div className="text-center py-4">
                    <div className="relative inline-block">
                      ...
                      <Button
                        ...
                        disabled={!location}
                        className="relative w-48 h-48 rounded-full bg-gradient-to-br from-red-500 to-red-600 hover:from-red-600 hover:to-red-700 text-white text-2xl font-bold shadow-[0_0_40px_rgba(239,68,68,0.4)] transform hover:scale-105 transition-all duration-200 border-4 border-red-400/30"
                      >
                        <div className="flex flex-col items-center space-y-2">
                          <Phone className="w-12 h-12" />
                          <span>SOS</span>
                          <span className="text-sm font-normal opacity-90">Get Help Now</span>
                        </div>
                      </Button>
                    </div>
                  </div>
                  ...
                  {selectedVehicle ? (
                    <div className="space-y-4">
                      <div className="flex items-center space-x-4">
                        <div className="w-16 h-16 bg-gradient-to-br from-emerald-500 to-emerald-600 rounded-lg flex items-center justify-center shadow-lg shadow-emerald-900/20">
                    ...
                    <div className="text-center py-8">
                      ...
                      <Link href="/vehicles">
                        <Button className="bg-gradient-to-r from-emerald-500 to-emerald-600 hover:from-emerald-600 hover:to-emerald-700 text-white shadow-lg shadow-emerald-900/20">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    ...
                    <Link key={index} href="/services">
                      <Card className="bg-slate-800/50 backdrop-blur-sm border-slate-700/50 hover:bg-slate-700/50 transition-all cursor-pointer shadow-lg hover:shadow-xl transform hover:scale-105 hover:-translate-y-1">
                        <CardContent className="p-4 text-center">
                          <div
                            className={`w-12 h-12 bg-gradient-to-br ${service.color} rounded-full flex items-center justify-center mx-auto mb-2 shadow-lg`}
                          >
                            <service.icon className="w-6 h-6 text-white" />
                          </div>
                          <p className="text-sm text-slate-200 font-medium">{service.label}</p>
                        </CardContent>
                      </Card>
                    </Link>
                  ))}
                ...
                {recentRequests.length > 0 ? (
                  <div className="space-y-4">
                    {recentRequests.map((request) => (
                      <div
                        key={request.id}
                        className="flex items-center justify-between p-4 bg-slate-800/50 backdrop-blur-sm rounded-lg border border-slate-700/50 hover:bg-slate-700/50 transition-colors"
                      >
